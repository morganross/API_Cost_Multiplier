# Timeline Failure Reason B4: MA Run Index Doesn't Increment
...
A significant issue contributing to the incomplete timeline is that the Multi-Agent (MA) run index does not increment across different models. All MA runs are logged with the same index, `[MA run 1]`, which leads to data corruption in the timeline generation script.
